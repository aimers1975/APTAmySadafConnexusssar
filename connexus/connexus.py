  ...
  def post(self):
    ...
    try:
      ...
      if (str(upload) == 'Upload'):
        ...
        templateVars = { "app_id" : AP_ID_GLOBAL, "other_html" : "" }
        templateVars2 = {"streamname":str(streamname),"badscript": BAD_SCRIPT,"picturelist":imagelinks, "rangestart":str(newstart),"rangeend":str(newend)}
...
class ViewAllStreamsService(webapp2.RequestHandler):
  ...
  def post(self):
    ...
    for stream in allstreamsbycreation:
      totalstreams.append(self.convertStreamObjToList(stream))
